...
Inputs:
args           arguments frmo command line object
Sim            int
AreaCells      double
NCells         int
Fire_Period    int
...
######################################################################################################################################
    ...
    while Sim <= TotalSims:
        ...
        '''
        if OutputGrid:
            #FI = np.zeros(len(FTypeCells2)).astype(int)    #FI = [0]*Max_Fire_Periods
            FI = {}
            HPeriod = [0]
                    ...
                    if StatusCells[aux] != 4 and (aux+1) not in BurntCells_Set:
                        ...
                        if Cells_Obj[aux].get_Status() == "Available" and Cells_Obj[aux].FType != 0:
                            if Cells_Obj[aux].ignition(Fire_Period[Year-1],Year,Ignitions,DF,coef_ptr,
                                                       args.Start_Threshold, args.HFI_Threshold):
                ...
                if Ignitions[Year] not in BurntCells_Set and StatusCells[Ignitions[Year]-1]!=4:
                    ...
                    if Cells_Obj[Ignitions[Year]-1].get_Status() == "Available" and Cells_Obj[Ignitions[Year]-1].FType != 0:
                        if Cells_Obj[Ignitions[Year]-1].ignition(Fire_Period[Year-1], Year, Ignitions, DF,
                                                                 coef_ptr, args.ROS_Threshold, args.HFI_Threshold):
                            ...
                            if verbose == True:
                                ...
                                print("Cell", Cells_Obj[Ignitions[Year]-1].ID, "Status:", Cells_Obj[Ignitions[Year]-1].get_Status())
                            
                            # Plot and save memory
                            if plottrue == True and SaveMem == True:
                                Plotter.forest_plotV3_FreeMem(Cells_Obj,emptylist,plotnumber,Fire_Period[Year-1],
                                                              Year,False,Rows,Cols,PlotPath,CoordCells,BurntCells_Set,Sim)
                                plotnumber+=1
                            
                            # Plot no save memory
                            if plottrue == True and SaveMem != True:
                                Plotter.forest_plotV3(Cells_Obj,emptylist,plotnumber,Fire_Period[Year-1],Year,False,
