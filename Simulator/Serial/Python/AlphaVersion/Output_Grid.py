...
def ScenarioOutput(TotalYears,Cells_Obj,NCells,BurntCells_Set,Folder,Sim,spotting,verbose):
	...
	for y in range(0,TotalYears):
		...
		#print "Fperiods:",FPeriods, "Sim:",Sim, "HPeriod:",HPeriods	
		if y >= 1:
...
def ExcelOutput(Cells_Obj,Sim,Folder,heuristic,TotalSims,TotalYears,NCells,BurntCells_Set,AvailCells_Set,spotting):
	#GlobalStats = [[] for i in range(0,TotalSims)]
	Sim2=Sim
	Sim = 1
	...
	#AveragesAvailNH = [[] for i in range(0,TotalSims)]
	#AveragesFireNH = [[] for i in range(0,TotalSims)]
	...
	# Percentages
	AveragesAvailNH[Sim-1] = round(len(AvailCells_Set)/float(NCells)*100.0,2)
	AveragesFireNH[Sim-1] = round(len(BurntCells_Set)/float(NCells)*100.0,2)
	
	if heuristic != 0:
		AveragesHarvestedH[Sim-1] = round(len(HarvestCells_Set2)/float(NCells)*100.0,2)
		AveragesAvailH[Sim-1] = round(len(AvailCells_Set2)/float(NCells)*100.0,2)
		AveragesFireH[Sim-1] = round(len(BurntCells_Set2)/float(NCells)*100.0,2)
	...
	if heuristic == True:
		ws1.cell(row = auxrow, column = 1, value = len(BurntCells_Set2))
	...
	# Compute statistics
	if heuristic != 0:
		TotalHFires = [0 for i in range(0,NCells)]
		TotalHHarvest = [0 for i in range(0,NCells)]
		AVGHFPeriod = [0 for i in range(0,NCells)]
		AVGHHPeriod = [0 for i in range(0,NCells)]
		ProbHFire = [0 for i in range(0,NCells)]
		ProbHHarvest = [0 for i in range(0,NCells)]
	
	TotalNHFires = [0 for i in range(0,NCells)]
	AVGNHFPeriod = [0 for i in range(0,NCells)]
	ProbNHFire = [0 for i in range(0,NCells)]
	
	for c in range(0,NCells):
		#TotalNHFires[c] += len(GlobalStats[TotalSims-1][0][c])
		...
		if heuristic != 0:	
			...
			ProbHFire[c] = TotalHFires[c]/float(TotalSims)
			ProbHHarvest[c] = TotalHHarvest[c]/float(TotalSims)
		
		#ProbNHFire[c] = TotalNHFires[c]/float(TotalSims)
		ProbNHFire[c] = TotalNHFires[c]/float(Sim)
					
		if TotalNHFires[c] != 0:
			#AVGNHFPeriod[c] = sum(GlobalStats[TotalSims-1][0][c])/float(TotalNHFires[c])
			AVGNHFPeriod[c] = sum(GlobalStats[Sim-1][0][c])/float(TotalNHFires[c])
		else:
			AVGNHFPeriod[c] = 0
		
		if heuristic != 0:
			if TotalHFires[c] != 0:
				#AVGHFPeriod[c] = sum(GlobalStats[TotalSims-1][1][c])/float(TotalHFires[c])
				AVGHFPeriod[c] = sum(GlobalStats[Sim-1][1][c])/float(TotalHFires[c])
			else:
				AVGHFPeriod[c] = 0
				
			if TotalHHarvest[c] != 0:
				#AVGHHPeriod[c] = sum(GlobalStats[TotalSims-1][2][c])/float(TotalHHarvest[c])
				AVGHHPeriod[c] = sum(GlobalStats[Sim-1][2][c])/float(TotalHHarvest[c])
			else:
				AVGHHPeriod[c]  = 0
		...
		if verbose == True:
			...
			#print "\n","Cell", c+1,"No Heur Fire:", GlobalStats[TotalSims-1][0][c]
			#print "Total NH Fires",c+1,"sim",TotalSims-1,":", TotalNHFires[c]
			...
			if heuristic != 0:
				#print "Cell", c+1,"Heur Fire:", GlobalStats[TotalSims-1][1][c]
				#print "Total H Fires",c+1,"sim",TotalSims-1,":", TotalHFires[c]	
				...
				#print "Cell", c+1,"Heur Harvested:", GlobalStats[TotalSims-1][2][c]
				#print "Total H Harvest",c+1,"sim",TotalSims-1,":", TotalHHarvest[c]	
			...
			print "Average of Available cells no heur: ", sum(AveragesAvailNH)/float(len(AveragesAvailNH)) 
			if heuristic != 0:
				print "Average of Available cells heur: ", sum(AveragesAvailH)/float(len(AveragesAvailH)) 
			print "Average of Burnt cells no heur: ", sum(AveragesFireNH)/float(len(AveragesFireNH)) 
			if heuristic != 0:
				print "Average of Burnt cells heur: ", sum(AveragesFireH)/float(len(AveragesFireH)) 
				print "Average of Harvested cells heur: ", sum(AveragesHarvestedH)/float(len(AveragesHarvestedH)) 
			
	if heuristic != 0:		
		ws1.cell(row=8, column=5, value = sum(AveragesAvailH)/float(len(AveragesAvailH)) )
		ws1.cell(row=9, column=5, value =  sum(AveragesHarvestedH)/float(len(AveragesHarvestedH)) )
		ws1.cell(row=10, column=5, value = sum(AveragesFireH)/float(len(AveragesFireH)))
	
	#print "A:",AveragesAvailNH
	ws1.cell(row=8, column=6, value = sum(AveragesAvailNH)/float(len(AveragesAvailNH)) )
	ws1.cell(row=9, column=6, value =  0)
	ws1.cell(row=10, column=6, value = sum(AveragesFireNH)/float(len(AveragesFireNH)))
	
	#Detailed frequency
	for c in range(0,NCells):
		for t in range(0,TotalYears):
			if heuristic != 0:
				#ws2.cell(row=c+3, column=2+t, value = GlobalStats[TotalSims-1][2][c].count(t+1))
				#ws2.cell(row=c+3, column=8+t, value = GlobalStats[TotalSims-1][1][c].count(t+1))
			#ws2.cell(row=c+3, column=14+t, value = GlobalStats[TotalSims-1][0][c].count(t+1))
