...
######################################################################################################################################
#
#                FireSimulator (Split) serial and parallel version 5.0 - Jan 2018 
...
###Step -1: FBP
    ...
    parser.add_argument("--input-fpblookup",
                    ...
                    type=str,
                    default=None) # "C:/Users/Lenovo/Desktop/SIM/Dogrib/fbp_lookup_table.csv"			
    ...
    if args.FBP_tester_cell > 0:
        ...
        mainstruct, headstruct, flankstruct, backstruct = ReadData.CalculateOne(DF,coef_ptr,cellID, verbose=True)
        quit()
    ...
    if verbose:
        print "Setting initial psuedo-random number stream seed to ",str(args.seed)
    ...
    while Sim <= TotalSims:
        ...
        if OutputGrid:
            ...
            HPeriod = [0]
        # BurntP is a list (by period) of the list of cells burned in the period
        ...
        while Year <= TotalYears:
            ...
            if rank == 0:
                ...
                ####Fire_Period[Year-1] = (week_number-1)*7*24+1 periods just counts minutes
                if verbose == True:
                    print ""
                    print "Current Period (Week): "+str(week_number)    
                    ...
                    while True:
                        ...
                        if StatusCells[aux] != 4 and (aux+1) not in BurntCells_Set:
                            if aux not in Cells_Obj.keys():
                                Cells_Obj[aux] = CellsFBP.Cells((aux+1),AreaCells,CoordCells[aux],AgeCells,FTypeCells[aux],coef_ptr[FTypes2[str.lower(CellsGrid4[aux])]],TerrainCells,VolCells,PerimeterCells,StatusCells[aux],AdjCells[aux],Colors[aux],RealCells[aux], OutputGrid)
                                '''
                ...
                if WeatherOpt != 'constant' and \
                   Fire_Period[Year-1] * FirePeriodLen / MinutesPerWP > weatherperiod:
                    weatherperiod +=1
                    DF = Weather_Obj.update_Weather_FBP(DF, WeatherOpt, weatherperiod)
            ...
            if NoIgnition == False:
                ...
                while Fire_Period[Year-1] < Max_Fire_Periods:
                    ...
                    #Serial version of the code: rank is mpi4py rank
                    if size == 1 and rank == 0:
                        ...
                        for cell in BurningCells_Set:
                            ...
                            if len(Cells_Obj[cell-1].ROSAngleDir) > 0:
                                ...
                                Aux_List = Cells_Obj[cell-1].manageFire(Fire_Period[Year-1],AvailCells_Set, verbose,DF,coef_ptr,spotting,SpottingParams, CoordCells,Cells_Obj, args)
                        ...
                        if rank == 0:
                            for bc in Global_Message_List:
                                if (bc-1) not in Cells_Obj.keys() and bc not in BurntCells_Set:
                                    Cells_Obj[bc-1] = CellsFBP.Cells((bc),AreaCells,CoordCells[bc-1],AgeCells,FTypeCells[bc-1],coef_ptr[FTypes2[str.lower(CellsGrid4[bc-1])]],TerrainCells,VolCells,PerimeterCells,StatusCells[bc-1],AdjCells[bc-1],Colors[bc-1],RealCells[bc-1], OutputGrid)
        ...
        if rank == 0:
            ...
            if exceltrue == True and SaveMem != True:
                ...
                # Call the function
                Output_Grid.ExcelOutput(Cells_Obj,Sim,OutFolder,heuristic,TotalSims,TotalYears,NCells,BurntCells_Set,AvailCells_Set)
            ...
            if SaveMem != True and scenarios==True:
                ...
                Output_Grid.ScenarioOutput(TotalYears,Cells_Obj,NCells,BurntCells_Set,OutFolder,Sim,spotting,verbose)
        ...
        if rank == 0 and OutputGrid == True:
            Output_Grid.OutputGrid(OutFolder,Rows,Cols,BurntCells_Set,Sim,spotting,verbose, AreaCells)
            if nooutput ==False:
