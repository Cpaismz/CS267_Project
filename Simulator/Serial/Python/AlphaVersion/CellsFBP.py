...
class Cells:
    ...
    def __init__(self,ID,Area,Coord,Age,FType,FType2,Terrain,Vol,Perimeter,Status,Adjacents,Color,RealID,OutputGrid):
        #Constructor
        self.ID = ID
        self.Area = Area
        self.Coord = Coord
    ...
    def manageFire(self,period,AvailSet, verbose,df,coef,spotting,SpottingParams,CoordCells,Cells_Obj, args):
        """
        some inputs:
        ...
        if  HROS > args.ROS_Threshold \
            and headstruct.fi > args.HFI_Threshold:
    ...
    def ignition(self,period,Season,IgnitionPoints,df,coef,ROSThresh, HFIThreshold):
        ...
        if IgnitionPoints != "":
            ...
            if headstruct.ros > ROSThresh and headstruct.fi > HFIThreshold:
                if verbose == True:
                    print ""
                    print "Head (ROS, FI) values of", str(headstruct.ros), str(headstruct.fi), " are enough for ignition"
                """
